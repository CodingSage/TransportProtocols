...
//function for calculating checksum given the packet
int calc_checksum(pkt packet)
{
	int sum = 0;
	for (int i = 0; i < PACKET_SIZE && packet.payload[i] != '\0'; i++)
...
//function for generating an ack packet on passing a seqnum which has to be ack'ed
pkt ack_packet(int seqnum)
{
	pkt ack;
	ack.acknum = seqnum;
	ack.seqnum = seqnum;
...
//function for generating ack packet for a particular received packet
pkt ack_packet(pkt packet)
{
	pkt ack;
...
void B_input(struct pkt packet)
{
	...
	{
		tolayer3(B, ack_packet(packet.seqnum));
		return;
	}
	buffer_B[packet.seqnum] = packet;
	ack_B[packet.seqnum] = true;
	tolayer3(B, ack_packet(packet));
	while (ack_B[base_B])
	{
		base_B++;
		tolayer5(B, buffer_B[base_B].payload);
		B_application++;
	}
}
